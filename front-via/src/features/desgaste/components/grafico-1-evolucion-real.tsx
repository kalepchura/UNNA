...
import type { Grafico1DesgasteResponse } from '../types/grafico-1.types';

interface Props {
  data: Grafico1DesgasteResponse | null;
  isLoading: boolean;
  height?: number;
}

export function Grafico1DesgasteEvolucionReal({ data, isLoading, height = 350 }: Props) {
  const chartRef = useRef<HTMLDivElement>(null);
  const chartContainerRef = useRef<HTMLDivElement>(null);
  const chartInstanceRef = useRef<ApexCharts | null>(null);

  const filename = `grafico-desgaste-1-${Date.now()}`;
  ...
  useEffect(() => {
    ...
    const options: ApexCharts.ApexOptions = {
      ...
      title: { text: 'Evolución del Desgaste Real', align: 'center', style: { fontSize: '16px', fontWeight: 'bold' } },
      xaxis: { type: 'numeric', title: { text: 'MTB Acumulado (millones de toneladas)', style: { fontWeight: 'bold' } } },
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader><CardTitle>Evolución del Desgaste Real</CardTitle></CardHeader>
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <CardTitle>Evolución del Desgaste Real</CardTitle>
