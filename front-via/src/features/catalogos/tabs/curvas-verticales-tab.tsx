...
function CeldaOpcionalDecimal({ valor, sufijo = '' }: { valor: number | null | undefined; sufijo?: string }) {
  if (valor === null || valor === undefined) {
    return <span className="text-muted-foreground text-xs">N.A.</span>;
  }
  return <span className="tabular-nums">{valor}{sufijo}</span>;
}
...
const columnas: ColumnDef<CurvaVertical>[] = [
  ...
  },
  {
    ...
    header: 'PK Vértice',
    cell: ({ row }) => <CeldaOpcional valor={row.original.pkVertice} sufijo=" m" />,
  },
  {
    accessorKey: 'pendienteEntrada',
    header: 'Pendiente Entrada',
    cell: ({ row }) => <CeldaOpcionalDecimal valor={row.original.pendienteEntrada} sufijo="‰" />,
  },
  {
    accessorKey: 'pendienteSalida',
    header: 'Pendiente Salida',
    cell: ({ row }) => <CeldaOpcionalDecimal valor={row.original.pendienteSalida} sufijo="‰" />,
  },
  {
    accessorKey: 'radio',
    header: 'Radio',
...
export function CurvasVerticalesTab() {
  ...
  const { data: todos, isLoading } = useApiQuery({
    ...
    queryFn: () => catalogosApi.curvasVerticales.listarParaTabla({ limit: 1000 }),
  });
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable
        ...
        data={datosPagina}
        loading={isLoading}
        mensajeVacio="No hay curvas verticales"
      />
